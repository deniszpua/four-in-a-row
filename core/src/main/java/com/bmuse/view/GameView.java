...
public class GameView extends GroupLayer implements MoveListener {
	...
	public GameView(FourInARow game, IDimension viewSize) {
		...
		float size = boardView.cellSize - 2, hsize = size/2;
		...
		.setStrokeColor(BLACK_COLOR).setStrokeWidth(STROKE_WIDTH).strokeCircle(size + hsize, hsize, hsize);
	...
	/**
	 * Method to add ball of specified color at given slot
	 */
